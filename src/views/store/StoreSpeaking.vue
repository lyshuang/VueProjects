...
<script type="text/ecmascript-6">
  ...
  import {findBook} from '../../utils/book'
  ...
  export default {
    ...
    methods: {
      ...
      togglePlay() {
        if (!this.isPlaying) {
          if (this.playStatus === 0) { //初次点击播放调用
            this.play()
          } else if (this.playStatus === 2) { //暂停后又点击播放调用
            this.continuePlay()
          }
        } else {
          this.pausePlay()
      ...
      continuePlay() {
        this.$refs.audio.play().then(() => {
          this.$refs.speakPlaying[0].startAnimation() //停止线条的动画
          this.isPlaying = true
          this.playStatus = 1
        })
      },
      ...
      createVoice(text) {
        //采用原生同步ajax方式，是为了兼容h5的audio，因为audio在苹果设备只能采用原生同步ajax方式
